...
const render = function (bundle, data) {
    const pagePath = pequeno.options.path;
    pequeno.log.verbose(
        `Rendering ${pagePath ? pagePath : path.basename(bundle)}`,
    );
    ...
    const addPage = function (component, data) {
        ...
        try {
            if (pagePath) {
                if (pagePath && data.route.href === pagePath) {
                    pushPage();
                }
            } else {
                pushPage();
