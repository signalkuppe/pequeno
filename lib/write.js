...
module.exports = async function ({ markup, styles, data, config, port }) {
    try {
        port.postMessage({
            level: 'verbose',
            msg: `Writing ${data.route.href}`,
        });
        ...
        const htmlOutput = `<!doctype html>${$.html()}`;
        const dest = path.join(config.outputDir, data.route.href);
        await fs.outputFile(dest, htmlOutput);
        port.postMessage({
            level: 'info',
            msg: `Writed ${data.route.href}`,
        });
    } catch (err) {
        port.postMessage({
            level: 'error',
            msg: `An error occurred while processing the markup ${data.route.href}, ${err}`,
        });
    }
};
